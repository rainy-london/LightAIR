...
class search_train_dataset(Dataset):
    def __init__(self, config, transform, action_list=None):
        self.image_root = config['image_root']
        self.transform = transform
        self.max_words = config['max_words']
        self.eda_p = config['eda_p']
        self.action_list = [a.lower().strip() for a in config['action_list']]
    ...
    def __getitem__(self, index):
        ...
        if 'anomaly' in ann and ann['anomaly'] is not None:
            action_text = str(ann['anomaly'])
        elif 'normal' in ann and ann['normal'] is not None:
            action_text = str(ann['normal'])
        
        if action_text:
            clean_text = action_text.lower().strip()
            try:
                action_label = self.action_list.index(clean_text)
            except ValueError:
                pass
        ...
        return image, caption, caption_eda, self.img_ids[img_id], pose, hard_i, hard_i_pose, hard_caption, action_label
...
class search_test_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.be_pose_img:
            if "multi-weather" in self.ann[index]['image']:
                pose_path = os.path.join(self.image_root, 'pose/test/' + self.ann[index]['image'].split('/')[-1])
                pose = Image.open(pose_path).convert('RGB')
                pose = self.transform(pose)
            else:
