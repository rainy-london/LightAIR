...
from dataset_TPS.utils import pre_caption
...
class re_train_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ann = self.ann[index]
        try:
            if 'CUHK-PEDES' in self.image_root:
...
class re_test_dataset(Dataset):
    ...
    def __getitem__(self, index):
        if 'CUHK' in self.image_root:
            image_name = self.ann[index]['image']#.split('/')[-1]
            image_path = os.path.join(self.image_root, image_name)
        else:
            image_name = self.ann[index]['image']
            image_path = os.path.join(self.image_root, image_name)
...
class re_train_dataset_ufine(Dataset):
    def __init__(self, config, transform, pre_transform):
        self.image_root = config['image_root']
        self.max_words = config['max_words']
        self.icfg_rstp = config['icfg_rstp']
        self.eda = config['eda']
        self.eda_p = config['eda_p']
        self.ifSpecial = config['ifSpecial']
        ann_file = config['train_file']
        ...
        raw_ann = []
        for f in ann_file:
            raw_ann += json.load(open(f, 'r'))
        if self.ifSpecial:
            raw_ann = [ann for ann in raw_ann if 'train' == ann['split']]

        for ann in raw_ann:
            if 'captions' in ann and isinstance(ann['captions'], list):
                for cap in ann['captions']:
                    new_ann = ann.copy()
                    new_ann['captions'] = [cap]
                    self.ann.append(new_ann)
            elif 'caption' in ann:
                new_ann = ann.copy()
                new_ann['captions'] = [ann['caption']]
                self.ann.append(new_ann)
            else:
                self.ann.append(ann)
        ...
        print("len train ann: ", len(self.ann))
        self.img_ids = {}
    ...
    def __getitem__(self, index):
        ...
        ann = self.ann[index]
        try:
            image_name = ann['file_path'].split('/')[-1]
            image_path = os.path.join(self.image_root, image_name)
        except:
            print("self.image_root", self.image_root)
            print("ann['file_path']", ann['file_path'])
        image = Image.open(image_path).convert('RGB')
        image1 = self.transform(image)

        caption = pre_caption(''.join(ann['captions']), self.max_words)
        if self.eda:
            caption1 = pre_caption(''.join(ann['captions']), self.max_words, self.icfg_rstp, True, self.eda_p)
            return image1, caption, caption1, self.img_ids[ann['id']]
        else:
            return image1, caption, self.img_ids[ann['id']]


class re_test_dataset_ufine(Dataset):
    def __init__(self, ann_file, config, transform):
        self.ifSpecial = config['ifSpecial']
        self.ann = json.load(open(ann_file, 'r'))
        self.transform = transform
        if self.ifSpecial:
            ...
            self.image_root = config.get('image_root_test')
        self.max_words = config['max_words']
        self.icfg_rstp = config['icfg_rstp']
        ...
        if self.ifSpecial:
            self.ann = [ann for ann in self.ann if 'test' == ann['split']]

        txt_id = 0
        for img_id, ann in enumerate(self.ann):
            if self.ifSpecial and ann['split'] != 'test':
                continue
            self.g_pids.append(ann['id'])
            self.image.append(ann['file_path'])
            self.img2txt[img_id] = []

            t = 0
            for i, caption in enumerate(ann['captions']):
                self.q_pids.append(ann['id'])
                self.text.append(pre_caption(caption, self.max_words, icfg_rstp=self.icfg_rstp))
                self.img2txt[img_id].append(txt_id)
                self.txt2img[txt_id] = []
                self.txt2img[txt_id].append(img_id)
            ...
            txt_id1 = 0
            for img_id1, ann1 in enumerate(self.ann):
                for i1, caption1 in enumerate(ann1['captions']):
                    if ann['id'] == ann1['id'] and img_id != img_id1:
                        self.img2txt[img_id].append(txt_id1)
                    txt_id1 += 1
                if ann['id'] == ann1['id'] and img_id != img_id1:
                    for temp in range(t):
    ...
    def __getitem__(self, index):
        image_name = self.ann[index]['file_path'].split('/')[-1]
        image_path = os.path.join(self.image_root, image_name)
        # image_path = os.path.join(self.image_root, self.ann[index]['image'])
        image = Image.open(image_path).convert('RGB')
        image = self.transform(image)
