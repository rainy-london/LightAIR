...
from dataset_TPS.re_dataset import re_train_dataset, re_test_dataset, re_test_dataset_icfg, re_train_dataset_attr, \
    re_test_dataset_attr, re_train_dataset_ufine, re_test_dataset_ufine
from dataset_TPS.randaugment import RandomAugment
from dataset_TPS.random_erasing import RandomErasing
...
def create_dataset(dataset, config, evaluate=False):
    ...
    gene_norm = cuhk_norm = icfg_norm = rstp_norm = pa100k_norm = normalize
    ...
    if dataset =='re_ufine':
        if config['ifSpecial']:
            test_dataset = re_test_dataset_ufine(config['train_file'][0], config, test_transform)
        else:
            test_dataset = re_test_dataset_ufine(config['test_file'],config, test_transform)
        if evaluate:
            return None, test_dataset
        train_dataset = re_train_dataset_ufine(config, train_transform, pre_transform)
