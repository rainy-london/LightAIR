...
class MaskedLMOutput(ModelOutput):
    """
    Output type of `RobertaForMaskedLM` model, containing:

    Args:
        loss (Optional[torch.FloatTensor], optional): Loss for the masked language modeling task. Only present if `labels` were provided during inference. Defaults to None.
        logits (torch.FloatTensor): Logits for the vocabulary items at masked positions.
        hidden_states (Optional[Tuple[torch.FloatTensor]], optional): Tuple of `torch.FloatTensor` (one for each layer) of hidden-states from the model. Defaults to None.
        attentions (Optional[Tuple[torch.FloatTensor]], optional): Tuple of `torch.FloatTensor` (one for each layer) containing the attention weights. Defaults to None.
        cross_attentions (Optional[Tuple[torch.FloatTensor]], optional): Tuple of `torch.FloatTensor` (one for each layer) containing the cross-attention weights. Defaults to None.
    """
